...
class TeamConversationController extends AbstractController
{
    ...
    public function invite(Request $request, Team $team, EntityManagerInterface $em, MailerInterface $mailer, TeamInvitationRepository $invitations): Response
    {
        ...
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $invitation = (new TeamInvitation())->setTeam($team)->setInvitedBy($this->getAppUser())->setEmail((string) $form->get('email')->getData());
            $em->persist($invitation);
            $em->flush();
            $url = $this->generateUrl('team_invitation_register', ['token' => $invitation->getToken()], UrlGeneratorInterface::ABSOLUTE_URL);
            $mailer->send((new TemplatedEmail())->from(new Address('no-reply@sjdb.space', 'sjdb.space'))->to($invitation->getEmail())->subject('Invitation à rejoindre le groupe '.$team->getName())->htmlTemplate('emails/team_invitation.html.twig')->context(['team' => $team, 'url' => $url]));
