...
# Retirer des mots spécifiques de la liste des stop words s'ils sont pertinents
...
def extract_keywords_combined_v1(text, corpus):
    processed_text = preprocess_text(text)

    if processed_text is None or processed_text.strip() == "":
        return []

    # Extraction initiale des mots-clés
    # Inclure les noms, verbes, noms propres, et adjectifs pour obtenir plus de mots pertinents
    keywords_spacy = [
    ...
    if not keywords_spacy:
     ...
     # Ajouter une composante de similarité de vecteurs pour trier les mots-clés
    ...
    # Trier les mots-clés par similarité décroissante
    sorted_keywords = sorted(keyword_scores, key=lambda x: x[1], reverse=True)
    return [kw[0] for kw in sorted_keywords if kw[1] > 0.3]  # Seuil de 0.3 pour la pertinence
...
def extract_keywords_combined(text, corpus):
    ...
    # Suppression de la partie qui fait la comparaison des similarités vectorielles
    # Juste retourner les mots-clés extraits pour l'instant
    return list(set(keywords_spacy))  # Utiliser `set` pour éliminer les doublons 
...
def analyze_context():
    ...
    # Étape 1 : Extraction des mots-clés (reprise de la méthode existante)
    processed_text = preprocess_text(user_message)
    keywords = [
        token.lemma_ for token in nlp(processed_text)
        if token.pos_ in ["NOUN", "VERB", "PROPN", "ADJ"]  # Ajout de ADJ pour inclure les adjectifs
    ]
    if not keywords:
        return jsonify({
    ...
    detected_intent = predict_intent(user_message)
    ...
    return jsonify({
        ...
        "context": detected_intent or "unknown",
        "entities": entities
    })

# Fonction de prétraitement du texte (exemple simplifié)
def preprocess_text(text):
    if not text:
        return ""
    doc = nlp(text)
