...
// Fonction pour charger les relations pondérées
...
// Ajuste la taille du SVG selon le conteneur
function resizeSVG() {
    const containers = document.querySelectorAll("#keywords-cluster, #statistics-visualization, #response-analysis");
...
// Ajout des interactions utilisateur sur les clusters
function addClusterInteractions() {
    svg.selectAll(".cluster")
        ...
        .on("click", function (event, d) {
            ...
            analyzeContext(clickedKeyword);
        });
}

// Affiche les clusters avec interactions
function drawClusters() {
    ...
    // Ajout des cercles pour chaque cluster
    clusterGroup.append("circle")
        .attr("cx", d => d.x)
        .attr("cy", d => d.y)
        .attr("r", 0) // Commence à 0 pour une animation
        .attr("fill", d => d.color)
        .attr("opacity", 0.7)
        .transition()
        .duration(1000)
    ...
    addClusterInteractions(); // Ajout des interactions sur les clusters
}
...
// Ajoute des liens pondérés entre clusters
function addWeightedLinks() {
    svg.selectAll(".weighted-link")
        ...
        .attr("opacity", d => d.weight)
        .attr("x1", d => clusters.find(c => c.keyword === d.source)?.x || 0)
        .attr("y1", d => clusters.find(c => c.keyword === d.source)?.y || 0)
        .attr("x2", d => clusters.find(c => c.keyword === d.target)?.x || 0)
        .attr("y2", d => clusters.find(c => c.keyword === d.target)?.y || 0);
}
...
// Génère des clusters enrichis à partir des mots-clés
function generateClustersFromKeywords(rawKeywords) {
    clusters.length = 0;
    particles.length = 0;
    const svg = d3.select("#keywords-cluster");
    svg.selectAll("*").remove(); // Efface les anciens éléments
    ...
    fetchRelationships(rawKeywords)
        .then(data => {
            ...
            // Enrichit les clusters pour D3.js
...
function renderStatistics(data) {
    ...
    const svg = d3.select("#statistics-visualization svg");
    svg.selectAll("*").remove();
    ...
    // Ajout des labels pour chaque barre
    svg.append("g")
        .selectAll("text")
        .data(intents)
        .enter()
